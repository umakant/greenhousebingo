...
export const LMS_STUDENT_ROLE_NAME = "lms-student";
const GUARD_NAME = "web";

/** LMS learner portal — self-service learning, not admin screens. */
export const LMS_STUDENT_PORTAL_PERMISSION_NAMES: readonly string[] = [
  "manage-dashboard",
  "manage-profile",
  "edit-profile",
...
export async function ensureLmsStudentRoleWithPermissions(): Promise<bigint> {
  let role = await prisma.role.findFirst({
    where: { name: LMS_STUDENT_ROLE_NAME, guardName: GUARD_NAME },
  ...
  } else if (role.label !== "LMS Student") {
    await prisma.role.update({
      where: { id: role.id },
      data: { label: "LMS Student" },
    });
  }
  ...
  for (const name of LMS_STUDENT_PORTAL_PERMISSION_NAMES) {
...
export async function assignLmsStudentRoleToUser(userId: bigint): Promise<void> {
  const roleId = await ensureLmsStudentRoleWithPermissions();
  ...
  await prisma.user.update({
    where: { id: userId },
    data: { type: "lms-student", isEnableLogin: true, isActive: true },
  });
}
