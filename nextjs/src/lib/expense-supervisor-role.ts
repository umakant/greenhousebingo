...
export const EXPENSE_SUPERVISOR_ROLE_NAME = "expense-supervisor";
const GUARD_NAME = "web";

export const EXPENSE_SUPERVISOR_PERMISSION_NAMES: readonly string[] = [
  "manage-dashboard",
  "manage-profile",
  "edit-profile",
  ...
  EM_WORKFLOW_PERMISSIONS.APPROVE,
];
...
export async function ensureExpenseSupervisorRoleWithPermissions(): Promise<bigint> {
  let role = await prisma.role.findFirst({
    where: { name: EXPENSE_SUPERVISOR_ROLE_NAME, guardName: GUARD_NAME },
  ...
  if (!role) {
    ...
    role = await prisma.role.create({
      data: {
        id: newId,
        name: EXPENSE_SUPERVISOR_ROLE_NAME,
        label: "Expense Supervisor",
        guardName: GUARD_NAME,
        editable: false,
      },
      select: { id: true, label: true },
    });
  } else if (role.label !== "Expense Supervisor") {
    await prisma.role.update({ where: { id: role.id }, data: { label: "Expense Supervisor" } });
  }

  await syncRolePermissions(role.id, [...EXPENSE_SUPERVISOR_PERMISSION_NAMES]);
  return role.id;
}
...
export async function assignExpenseSupervisorRoleToUser(userId: bigint): Promise<void> {
  const roleId = await ensureExpenseSupervisorRoleWithPermissions();
  ...
  await prisma.user.update({
    where: { id: userId },
    data: { type: "expense-supervisor", isEnableLogin: true, isActive: true },
  });
}
