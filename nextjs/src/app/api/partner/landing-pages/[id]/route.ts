...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const page = await prisma.partnerLandingPage.findFirst({
    where: { id: pageId, partnerId: guard.partner.id },
  });
  ...
  if (body.slug != null) {
    ...
    if (wanted && wanted !== page.slug) {
      data.slug = await ensureUniqueLandingSlug(guard.partner.id, wanted);
    }
  }
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.partnerLandingPage.deleteMany({ where: { id: pageId, partnerId: guard.partner.id } });
