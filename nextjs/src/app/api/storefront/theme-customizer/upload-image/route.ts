...
/** See `deploy/nextjs.nginx.example` if uploads fail with HTTP 413 at the reverse proxy. */
...
/** Hero / customizer images — same storage as catalog; permission matches theme editor. */
export async function POST(req: NextRequest) {
  const denied = await assertStorefrontApiAccess(req, {
    permission: STOREFRONT_PERMISSION.THEME_MANAGE,
  ...
  const relDir = `uploads/storefront-theme-customizer/${orgSeg}`;
  ...
  for (const f of files) {
    ...
    const filename = `hero-${Date.now()}-${crypto.randomBytes(5).toString("hex")}${safeExt}`;
    ...
    if (useCloudinary) {
      try {
        ...
        const folder = `${baseFolder}/storefront-theme-customizer/${orgSeg}`;
