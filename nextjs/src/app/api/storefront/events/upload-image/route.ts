...
/**
 * Storefront events image upload — same Cloudinary-or-local pipeline as the blog featured-image
 * endpoint. Permission and size cap mirror the blog admin (PAGE_MANAGE).
 */
export async function POST(req: NextRequest) {
  ...
  if (files.length > 4) {
    return NextResponse.json({ ok: false, message: "Too many files at once (max 4)." }, { status: 400 });
  }
  ...
  const relDir = `uploads/storefront-events/${orgSeg}`;
  ...
  for (const f of files) {
    ...
    const filename = `event-${Date.now()}-${crypto.randomBytes(5).toString("hex")}${safeExt}`;
    ...
    if (useCloudinary) {
      try {
        ...
        const folder = `${baseFolder}/storefront-events/${orgSeg}`;
