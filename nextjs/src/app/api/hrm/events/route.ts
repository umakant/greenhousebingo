...
export async function GET(req: NextRequest) {
  ...
  try {
    const [rows, total] = await Promise.all([
      prisma.hrmEvent.findMany({ where, orderBy: { startAt: "desc" }, skip, take: perPage }),
      prisma.hrmEvent.count({ where }),
    ]);
    return jsonR({
...
export async function POST(req: NextRequest) {
  ...
  if (!body?.title || !body?.start_at) return NextResponse.json({ error: "title and start_at required" }, { status: 400 });
  try {
    const row = await prisma.hrmEvent.create({
      data: {
        title: body.title,
