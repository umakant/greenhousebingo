...
export async function GET(req: NextRequest) {
  const perms = getHrmPerms(req); if (!checkPerm(perms, "manage-hrm", "manage-leave-types", "manage-leave-applications")) return forbidden();
  ...
  try {
    const [rows, total] = await Promise.all([prisma.hrmLeaveType.findMany({ where, orderBy: { createdAt: "desc" }, skip, take: perPage }), prisma.hrmLeaveType.count({ where })]);
...
export async function POST(req: NextRequest) {
  const perms = getHrmPerms(req); if (!checkPerm(perms, "manage-leave-types", "create-leave-types")) return forbidden();
  const actor = await getHrmActor(req); if (!actor) return unauthorized();
  const body = await req.json().catch(() => null);
  if (!body?.name) return NextResponse.json({ error: "name required" }, { status: 400 });
  try {
    const row = await prisma.hrmLeaveType.create({ data: { name: body.name, daysAllowed: body.days_allowed ?? 0, leaveCode: body.leave_code ?? null, isActive: body.is_active ?? true, createdBy: getCompanyId(actor) } });
