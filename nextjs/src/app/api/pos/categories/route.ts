...
export async function GET() {
  ...
  const rows = await prisma.posCategory.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const row = await prisma.posCategory.create({ data: { name: body.name.trim(), description: body.description || null } });
