...
export async function GET() {
  ...
  const rows = await prisma.posBrand.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const row = await prisma.posBrand.create({ data: { name: body.name.trim(), description: body.description || null } });
