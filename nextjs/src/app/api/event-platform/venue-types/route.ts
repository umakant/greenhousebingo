...
import { createVenueType, listVenueTypes } from "@/lib/event-platform/venues/venue-lookup-service";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const items = await listVenueTypes(actor.organizationId, activeOnly);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const item = await createVenueType(
