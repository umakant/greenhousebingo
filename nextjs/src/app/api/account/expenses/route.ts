...
async function genRef(companyId: bigint): Promise<string> {
  const count = await prisma.expense.count({ where: { createdBy: companyId } });
  return "EXP-" + String(count + 1).padStart(4, "0");
}

export async function GET(req: NextRequest) {
  const perms = getPermissionsFromCookieValue(req.cookies.get("pf_permissions")?.value);
  if (!hasAccountPermission(perms, "manage-expenses")) {
  ...
  const [total, rows] = await Promise.all([
    ...
    prisma.expense.findMany({ where, orderBy: { date: "desc" }, skip: (page - 1) * perPage, take: perPage }),
  ]);

  const vendorIds = [...new Set(rows.map((r) => r.vendorId).filter(Boolean))] as bigint[];
  const vendors = vendorIds.length
    ? await prisma.vendor.findMany({ where: { id: { in: vendorIds } }, select: { id: true, name: true, companyName: true } })
    : [];
