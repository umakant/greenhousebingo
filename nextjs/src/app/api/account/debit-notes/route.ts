...
async function genRef(companyId: bigint): Promise<string> {
  const count = await prisma.debitNote.count({ where: { createdBy: companyId } });
  return "DN-" + String(count + 1).padStart(4, "0");
}

export async function GET(req: NextRequest) {
  const perms = getPermissionsFromCookieValue(req.cookies.get("pf_permissions")?.value);
  if (!hasAccountPermission(perms, "manage-debit-notes")) {
  ...
  const [total, rows] = await Promise.all([
    ...
    prisma.debitNote.findMany({ where, orderBy: { date: "desc" }, skip: (page - 1) * perPage, take: perPage }),
  ]);
  ...
  const data = rows.map((r) => ({
    ...
    vendor: r.vendorId ? (vMap[String(r.vendorId)] ?? null) : null,
  }));
...
export async function POST(req: NextRequest) {
  const perms = getPermissionsFromCookieValue(req.cookies.get("pf_permissions")?.value);
  if (!hasAccountPermission(perms, "manage-debit-notes")) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const actorEmail = normalizeEmail(req.cookies.get("pf_email")?.value ?? "");
  if (!actorEmail) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const actor = await prisma.user.findFirst({ where: { email: actorEmail }, select: { id: true, type: true, createdBy: true } });
  if (!actor?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  ...
  const record = await prisma.debitNote.create({
    data: {
      referenceNumber,
      vendorId: body.vendor_id != null ? BigInt(Number(body.vendor_id)) : null,
