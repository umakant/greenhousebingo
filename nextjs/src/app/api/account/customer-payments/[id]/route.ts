...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.customerPayment.findFirst({ where: { id: BigInt(id), createdBy: companyId } });
  ...
  await prisma.customerPayment.update({
    where: { id: BigInt(id) },
    data: {
      ...(body.customer_id != null && { customerId: BigInt(Number(body.customer_id)) }),
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.customerPayment.findFirst({ where: { id: BigInt(id), createdBy: companyId } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
  await prisma.customerPayment.delete({ where: { id: BigInt(id) } });
  return NextResponse.json({ ok: true });
}
