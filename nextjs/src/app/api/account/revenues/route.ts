...
export async function GET(req: NextRequest) {
  ...
  if (!hasAccountPermission(perms, "manage-revenues")) {
  ...
  const [total, rows] = await Promise.all([
    ...
    prisma.revenue.findMany({ where, orderBy: { date: "desc" }, skip: (page - 1) * perPage, take: perPage }),
  ]);
  ...
  const customers = customerIds.length
    ...
    : [];
  const cMap = Object.fromEntries(customers.map((c) => [String(c.id), { company_name: c.companyName }]));
...
export async function POST(req: NextRequest) {
  const perms = getPermissionsFromCookieValue(req.cookies.get("pf_permissions")?.value);
  if (!hasAccountPermission(perms, "manage-revenues")) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  const record = await prisma.revenue.create({
    data: {
      referenceNumber,
