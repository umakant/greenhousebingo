...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.revenue.findFirst({ where: { id: BigInt(id), createdBy: companyId } });
  ...
  await prisma.revenue.update({
    where: { id: BigInt(id) },
    data: {
      ...(body.customer_id !== undefined && { customerId: body.customer_id != null ? BigInt(Number(body.customer_id)) : null }),
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.revenue.findFirst({ where: { id: BigInt(id), createdBy: companyId } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
  await prisma.revenue.delete({ where: { id: BigInt(id) } });
  return NextResponse.json({ ok: true });
}
