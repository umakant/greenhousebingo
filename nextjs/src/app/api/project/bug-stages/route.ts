...
export async function GET(req: NextRequest) {
  ...
  const stages = await prisma.bugStage.findMany({
...
export async function POST(req: NextRequest) {
  ...
  const maxOrder = await prisma.bugStage.aggregate({ where: { createdBy: companyId }, _max: { order: true } });
  const nextOrder = (maxOrder._max.order ?? 0) + 1;

  const stage = await prisma.bugStage.create({
    data: {
      name: String(body.name).trim(),
      color: body.color ? String(body.color) : "#77b6ea",
