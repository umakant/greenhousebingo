...
async function getActor(req: NextRequest) {
  ...
  return prisma.user.findFirst({ where: { email }, select: { id: true, type: true, createdBy: true, name: true } });
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const comment = await prisma.taskComment.create({
    data: { taskId: BigInt(id), comment: String(body.comment).trim(), userId: actor.id },
  });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.taskComment.delete({ where: { id: BigInt(commentId) } }).catch(() => {});
