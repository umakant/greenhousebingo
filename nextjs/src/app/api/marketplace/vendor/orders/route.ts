...
function serializeListOrder(
  ...
  const deliveryWindow = start && end ? `${start} – ${end}` : start || null;
...
export async function GET(req: NextRequest) {
  ...
  const [orders, vendor, allForSummary] = await Promise.all([
  ...
  const [buyers, eventLinks, deliveries] = await Promise.all([
    ...
    orderIds.length
      ? prisma.deliveryEventOrder.findMany({
          ...
          include: {
            deliveryEvent: { select: { deliveryDate: true, startTime: true, endTime: true } },
          },
          orderBy: { createdAt: "desc" },
        })
      : Promise.resolve([]),
  ...
  if (tab !== "all") items = items.filter((o) => matchesOrderTab(o, tab));
  ...
  const tabCounts = Object.fromEntries(
    ORDER_TABS.map((t) => [
      t.key,
      t.key === "all" ? allForSummary.length : allForSummary.filter((o) => matchesOrderTab(o, t.key)).length,
    ]),
