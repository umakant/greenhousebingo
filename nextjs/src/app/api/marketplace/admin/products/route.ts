...
export async function GET(req: NextRequest) {
  ...
  const [products, categories, vendors, summaryAgg] = await Promise.all([
...
export async function POST(req: NextRequest) {
  ...
  const vendorIdRaw = body.vendorId ?? body.vendor_id;
  let vendorId: bigint;
  try {
    vendorId = BigInt(String(vendorIdRaw));
  } catch {
    return NextResponse.json({ ok: false, message: "A valid vendor is required." }, { status: 400 });
  }
  const vendor = await prisma.marketplaceVendor.findFirst({ where: { id: vendorId }, select: { id: true } });
  if (!vendor) {
