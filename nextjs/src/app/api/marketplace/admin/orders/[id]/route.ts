...
import { MARKETPLACE_ORDER_STATUSES, serializeDelivery, serializeOrder } from "@/lib/marketplace-service";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const order = await prisma.marketplaceOrder.findFirst({
    ...
    include: {
      ...
      items: {
        include: {
          product: { select: { imageUrl: true, image: true } },
        },
      },
  ...
  const lineItems = (order.items.length ? order.items : order.lines).map((row) => {
    ...
    return {
      ...
      sku: isItem && "productId" in row ? null : null,
    };
  });
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  const denied = await guardMarketplaceAdmin(req, "marketplace.orders.manage");
  if (denied) return denied;
  ...
  if (body.status != null) {
    ...
    if (!MARKETPLACE_ORDER_STATUSES.includes(status as (typeof MARKETPLACE_ORDER_STATUSES)[number])) {
      return NextResponse.json({ ok: false, message: "Invalid order status." }, { status: 400 });
    }
    data.status = status;
  }
  if (body.paymentStatus != null) {
    data.paymentStatus = String(body.paymentStatus).trim() || "unpaid";
  }
