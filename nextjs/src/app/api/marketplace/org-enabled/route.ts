...
import { readMarketplaceOrgEnabled, writeMarketplaceOrgEnabled } from "@/lib/marketplace-organization";
...
function isAuthorized(isSuper: boolean, perms: string[]): boolean {
  return (
    isSuper ||
    perms.includes("*") ||
    hasPermission(perms, "marketplace.settings.manage") ||
    hasPermission(perms, "marketplace.manage")
  );
}

/** Superadmin or tenant user with `marketplace.settings.manage` may read Marketplace org opt-in state. */
export async function GET(req: NextRequest) {
  ...
  const enabled = await readMarketplaceOrgEnabled(organizationId);
  return NextResponse.json({
    ok: true,
...
export async function PATCH(req: NextRequest) {
  ...
  const prev = await readMarketplaceOrgEnabled(organizationId);
  await writeMarketplaceOrgEnabled(organizationId, enabled);

  if (prev !== enabled) {
    await writeSaasAuditLog({
      eventType: "marketplace_org_toggle",
      module: "Marketplace",
