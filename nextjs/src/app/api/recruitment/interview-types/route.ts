...
export async function GET(req: NextRequest) {
  ...
  if (!checkPerm(perms, "manage-recruitment", "manage-recruitment-system-setup", "manage-interviews")) return forbidden();
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.recInterviewType.findMany({ where, orderBy: { createdAt: "desc" }, skip: (page - 1) * perPage, take: perPage }),
      prisma.recInterviewType.count({ where }),
    ]);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rec = await prisma.recInterviewType.create({
