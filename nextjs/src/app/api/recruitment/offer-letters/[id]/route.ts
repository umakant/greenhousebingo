...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rec = await prisma.recOfferLetter.findFirst({ where: { id: BigInt(id), createdBy: cid } });
    if (!rec) return notFound();
    const body = await req.json();
    const updated = await prisma.recOfferLetter.update({ where: { id: BigInt(id) }, data: { lang: body.lang ?? rec.lang, content: body.content ?? rec.content } });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rec = await prisma.recOfferLetter.findFirst({ where: { id: BigInt(id), createdBy: cid } });
    if (!rec) return notFound();
    await prisma.recOfferLetter.delete({ where: { id: BigInt(id) } });
