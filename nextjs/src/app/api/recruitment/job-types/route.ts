...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.recJobType.findMany({ where, orderBy: { createdAt: "desc" }, skip: (page - 1) * perPage, take: perPage }),
      prisma.recJobType.count({ where }),
    ]);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rec = await prisma.recJobType.create({
