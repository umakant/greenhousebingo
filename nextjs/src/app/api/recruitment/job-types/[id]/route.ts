...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rec = await prisma.recJobType.findFirst({ where: { id: BigInt(id), createdBy: cid } });
    if (!rec) return notFound();
    const body = await req.json();
    const updated = await prisma.recJobType.update({ where: { id: BigInt(id) }, data: { name: body.name ?? rec.name, description: body.description ?? null, isActive: body.is_active ?? rec.isActive } });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rec = await prisma.recJobType.findFirst({ where: { id: BigInt(id), createdBy: cid } });
    if (!rec) return notFound();
    await prisma.recJobType.delete({ where: { id: BigInt(id) } });
