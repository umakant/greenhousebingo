...
type MaintenanceRow = {
  ...
  cost?: string | null; technicianName?: string | null; scheduledDate?: string | null;
  asset?: { id: string; name: string } | null;
};

const emptyForm = {
  asset_id: "__none__", type: "Preventive", title: "", description: "",
  scheduled_date: "", completed_date: "", cost: "", technician_name: "",
  status: "Scheduled", priority: "Medium", next_maintenance_date: "", notes: "",
};
...
function priorityBadge(p: string) {
  ...
  return <span className={`px-2 py-1 rounded-full text-xs font-medium ${map[p] ?? "bg-gray-100 text-gray-700"}`}>{p}</span>;
}
...
function statusBadge(s: string) {
  ...
  const labels: Record<string, string> = { Scheduled: "Scheduled", InProgress: "In Progress", Completed: "Completed", Cancelled: "Cancelled" };
  return <span className={`px-2 py-1 rounded-full text-xs font-medium ${map[s] ?? "bg-gray-100 text-gray-700"}`}>{labels[s] ?? s}</span>;
}

export default function AssetsMaintenance({ permissions }: { permissions: string[] }) {
  ...
  const fmtCur = (v: number | null | undefined) => v != null ? formatCurrency(v, settings) : "—";
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/assets/maintenance?${params}`);
  ...
  function openEdit(row: MaintenanceRow) {
    ...
    setForm({
      ...
      title: row.title,
      description: "",
      scheduled_date: row.scheduledDate?.slice(0, 10) ?? "",
      completed_date: "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setProcessing(true);
    try {
      const url = mode === "add" ? "/api/assets/maintenance" : `/api/assets/maintenance/${editId}`;
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/assets/maintenance/${deleteId}`, { method: "DELETE" });
      const json = await res.json();
      if (!res.ok) { toast.error(json.error ?? "Failed"); return; }
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        {can("create-asset-maintenance") && (
          <Button onClick={openAdd} className="gap-2"><Plus className="h-4 w-4" />{t("Add Maintenance")}</Button>
        )}
      </div>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-16">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-muted-foreground">{t("No maintenance records found")}</p>
                      </div>
                    </td>
                  </tr>
                ) : items.map(row => (
                  <tr key={row.id} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{row.asset?.name ?? "—"}</td>
                    <td className="px-4 py-3">{row.type}</td>
                    <td className="px-4 py-3 text-muted-foreground">{row.scheduledDate ? fmtDate(row.scheduledDate) : "—"}</td>
                    <td className="px-4 py-3">{priorityBadge(row.priority)}</td>
                    <td className="px-4 py-3">{statusBadge(row.status)}</td>
                    <td className="px-4 py-3">{fmtCur(row.cost != null ? Number(row.cost) : null)}</td>
                    <td className="px-4 py-3">
                      <TableActionButton label={t("Actions")} items={[
                        ...(can("edit-asset-maintenance") ? [{ label: t("Edit"), icon: <Pencil className="h-4 w-4" />, onSelect: () => openEdit(row) }] : []),
                        ...(can("delete-asset-maintenance") ? [{ label: t("Delete"), icon: <Trash2 className="h-4 w-4" />, onSelect: () => setDeleteId(row.id), destructive: true }] : []),
                      ]} />
                    </td>
                  </tr>
                ))}
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{mode === "add" ? t("Add Maintenance") : t("Edit Maintenance")}</SheetTitle>
          </SheetHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label>{t("Cost")}</Label>
                <Input type="number" step="0.01" value={form.cost} onChange={e => f("cost", e.target.value)} placeholder="0.00" />
              </div>
              <div className="space-y-1">
                <Label>{t("Technician Name")}</Label>
                <Input value={form.technician_name} onChange={e => f("technician_name", e.target.value)} />
              </div>
            </div>
            <div className="space-y-1">
            ...
            <div className="flex gap-2 pt-2">
              <Button type="submit" disabled={processing} className="flex-1">
                {processing ? t("Saving...") : mode === "add" ? t("Create Maintenance") : t("Update Maintenance")}
              </Button>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={o => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Delete Maintenance Record")}</AlertDialogTitle>
            <AlertDialogDescription>{t("This will permanently delete this maintenance record.")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
