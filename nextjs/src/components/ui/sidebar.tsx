...
>(
  (
    ...
    /** Must match SSR: do not read `document.cookie` in the initializer (server has no cookie → defaultOpen; client would diverge and cause layout jump). Sync cookie before paint. */
    const [_open, _setOpen] = React.useState(defaultOpen)
    React.useLayoutEffect(() => {
      const m = document.cookie.match(new RegExp(`${SIDEBAR_COOKIE_NAME}=(true|false)`))
      if (m?.[1] === "true") _setOpen(true)
      else if (m?.[1] === "false") _setOpen(false)
    }, [])
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              /* Main content (SidebarInset) must use `text-foreground`; only the sidebar rail uses sidebar tokens. */
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-background text-sidebar-foreground shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring placeholder:text-sidebar-foreground/50",
        className
      )}
      {...props}
    />
