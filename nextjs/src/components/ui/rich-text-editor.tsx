...
export function RichTextEditor({ content = "", onChange, placeholder, className, disabled = false, onKeyDown }: Props) {
  const editor = useEditor({
    // TipTap recommends disabling immediate render in React 18 / hydration environments.
    ...
    editorProps: {
      attributes: {
        class:
          "focus:outline-none min-h-[120px] p-3 prose prose-sm max-w-none [&_ul]:list-disc [&_ul]:ml-6 [&_ol]:list-decimal [&_ol]:ml-6 [&_li]:my-1",
      },
      handleKeyDown: (_view, event) => {
        if (onKeyDown) onKeyDown(event as unknown as React.KeyboardEvent);
        return false;
      },
    },
    editable: !disabled,
  ...
  React.useEffect(() => {
    if (!editor) return;
    // Keep editor in sync when parent resets content.
  ...
  return (
    <div className={cn("border rounded-md", className)}>
      ...
      <div className="prose prose-sm max-w-none [&_ul]:list-disc [&_ul]:ml-6 [&_ol]:list-decimal [&_ol]:ml-6 [&_li]:my-1 [&_a]:text-blue-600 [&_a]:underline [&_a]:cursor-pointer hover:[&_a]:text-blue-800">
        {/* placeholder is handled by tiptap placeholder extension in Laravel; keep prop for parity */}
        <EditorContent editor={editor} data-placeholder={placeholder ?? t("Start typing...")} />
      </div>
    </div>
  );
}
