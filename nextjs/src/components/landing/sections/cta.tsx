...
export default function CTA({ settings }: { settings?: any }) {
  ...
  const config = (CTA_VARIANTS as any)[variant] || CTA_VARIANTS.cta1;

  const title = sectionData.title || "Ready to Transform Your Business?";
  const subtitle =
    sectionData.subtitle ||
    "Join thousands of businesses already using WorkDo Dash to streamline their operations.";
  ...
  const getBackgroundStyle = () => {
    if (config.layout === "centered") return { backgroundColor: colors.primary };
  ...
  const renderButtons = () => {
    const primaryLink = sectionData.primary_button_link || "/register";
    const secondaryLink = sectionData.secondary_button_link || "/login";
              ...
              {sectionData.image ? (
                ...
                <div className="bg-gradient-to-br from-gray-100 to-gray-200 h-80 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-slate-500 font-medium">Upload CTA Image</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative z-10 space-y-8">
              <div className="space-y-6">
                <div
                  ...
                  style={{ backgroundColor: `${colors.primary}15`, border: `2px solid ${colors.primary}25` }}
                >
              ...
              <div className="pt-4">{renderButtons()}</div>
            </div>
          </div>
        </div>
      </section>
  ...
  return (
    <section className={config.section} style={getBackgroundStyle()}>
      {config.layout === "gradient" && <div className="absolute inset-0 bg-black/20"></div>}
