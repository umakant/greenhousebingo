...
const HERO_VARIANTS = {
  hero1: {
    ...
    primaryBtn:
      "text-white px-8 py-3 rounded-lg text-lg font-medium flex items-center transition-all duration-300 shadow-lg hover:shadow-xl",
    secondaryBtn:
      "border-2 border-gray-300 text-slate-700 px-8 py-3 rounded-lg text-lg font-medium hover:bg-gray-50 transition-all duration-300",
    layout: "right-split",
    showImage: true,
  },
  hero2: {
    ...
    primaryBtn:
      "text-white px-8 py-3 rounded-md text-lg font-medium flex items-center justify-center transition-colors",
  ...
  hero3: {
    ...
    primaryBtn:
      "text-white px-10 py-4 rounded-2xl text-lg font-semibold flex items-center justify-center transition-all duration-300 shadow-2xl hover:shadow-3xl transform hover:-translate-y-1 backdrop-blur-sm",
    secondaryBtn:
      "border-2 border-white/30 text-white px-10 py-4 rounded-2xl text-lg font-semibold hover:bg-white/10 hover:border-white/50 transition-all duration-300 backdrop-blur-sm",
    layout: "background",
    showImage: false,
  },
  hero4: {
    ...
    secondaryBtn:
      "text-slate-700 px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-all duration-200 border border-gray-200 hover:border-gray-300",
    layout: "minimal",
    showImage: false,
  },
} as const;
...
export default function Hero({ settings }: { settings?: any }) {
  ...
  const config = (HERO_VARIANTS as any)[variant] || HERO_VARIANTS.hero1;

  const companyName = settings?.company_name || "PaperFlight";
  const rawTitle = sectionData.title || `Transform Your Business with ${companyName}`;
  const title = rawTitle.replace(/\bWorkDo Dash\b/gi, companyName);
  const subtitle =
  ...
  const primaryButtonLink = sectionData.primary_button_link || "/register";
  const secondaryButtonText = sectionData.secondary_button_text || "Request Demo";
  const secondaryButtonLink = sectionData.secondary_button_link || "/login";
  const storedHighlight = sectionData.highlight_text ?? "";
  const highlightText =
    storedHighlight && storedHighlight !== "WorkDo Dash" ? storedHighlight : companyName;
  ...
  const renderButtons = () => (
    <div className={config.buttons}>
      ...
      <button className={config.secondaryBtn} onClick={() => (window.location.href = secondaryButtonLink)}>
        {secondaryButtonText}
      </button>
    </div>
  );
    ...
    return (
      <div className="bg-gray-100 rounded-xl overflow-hidden shadow-lg">
        {heroImage ? (
          ...
          <div className="w-full h-full min-h-64 flex items-center justify-center text-gray-500">Hero Image</div>
        )}
      </div>
    );
  };
