...
import { Building2, Calculator, CreditCard, FolderOpen, Users, UserCheck, Package, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Features({ settings }: { settings?: any }) {
  ...
  const config = (FEATURES_VARIANTS as any)[variant] || FEATURES_VARIANTS.features1;
  ...
  const getIcon = (iconName: string) => {
    const icons = { Building2, Calculator, Users, CreditCard, UserCheck, FolderOpen, Package };
    return (icons as any)[iconName] || Building2;
  };
      ...
      <section className={config.section}>
        <div className={config.container}>
          <div className={config.grid}>
            ...
            <div className="space-y-4">{features.map((f: any, idx: number) => renderFeature(f, idx))}</div>
          </div>
        </div>
      </section>
    );
        ...
        <div className={config.container}>
          ...
          <div className="relative">
            <div className={config.grid}>
              ...
              >
                {duplicatedFeatures.map((f: any, idx: number) => renderFeature(f, idx))}
              </div>
            </div>
            <button
              onClick={prevSlide}
            ...
            <div className="flex justify-center mt-8 space-x-2">
              {features.map((_: any, idx: number) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${currentSlide === idx ? "scale-125" : "hover:scale-110"}`}
                  style={{ backgroundColor: currentSlide === idx ? colors.primary : "#d1d5db" }}
  ...
  return (
    <section className={config.section}>
      <div className={config.container}>
        <h2 className={config.title}>{title}</h2>
        <p className={config.subtitle}>{subtitle}</p>
        <div className={config.grid}>{features.map((f: any, idx: number) => renderFeature(f, idx))}</div>
      </div>
    </section>
  );
}
