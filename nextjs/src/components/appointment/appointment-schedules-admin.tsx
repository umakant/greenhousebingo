...
import { Trash2, Calendar, Search, CheckCircle, XCircle, Clock } from "lucide-react";
...
type SchRow = {
  id: string; uniqueId: string; name: string | null; email: string | null; phone: string | null;
  date: string | null; startTime: string | null; endTime: string | null;
  status: string; cancelDescription: string | null;
  appointment: { id: string; appointmentName: string } | null;
};

export default function AppointmentSchedulesAdmin({ permissions }: { permissions: string[] }) {
  ...
  const [rows, setRows] = React.useState<SchRow[]>([]);
  ...
  const [viewRow, setViewRow] = React.useState<SchRow | null>(null);
  const [actionOpen, setActionOpen] = React.useState(false);
  const [actionRow, setActionRow] = React.useState<SchRow | null>(null);
  ...
  async function load() {
    ...
    try {
      let url = `/api/appointment/schedules?page=${page}&per_page=${perPage}`;
  ...
  function openAction(row: SchRow, status: string) {
    setActionRow(row); setActionStatus(status); setCancelDesc(""); setActionOpen(true);
  }
  ...
  async function applyAction(e: React.FormEvent) {
    ...
    try {
      ...
      if (actionStatus === "cancelled" || actionStatus === "rejected") body.cancel_description = cancelDesc;
      const res = await fetch(`/api/appointment/schedules/${actionRow.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function del(id: string) {
    if (!(await appConfirm(t("Delete this schedule?")))) return;
    const res = await fetch(`/api/appointment/schedules/${id}`, { method: "DELETE" });
        ...
        <CardContent className="p-4 border-b bg-gray-50/50">
          <div className="flex gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input className="pl-9" placeholder={t("Search by name, email...")} value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && load()} />
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-muted-foreground">
                <tr className="border-b">
                  ...
                  <th className="text-left font-medium px-4 py-3">{t("Time")}</th>
                  ...
                  : rows.length === 0 ? (
                    ...
                    return (
                      <tr key={row.id} className="border-b hover:bg-accent/20">
                        ...
                        <td className="px-4 py-3">{row.startTime ?? "-"}{row.endTime ? ` – ${row.endTime}` : ""}</td>
                        <td className="px-4 py-3"><Badge variant={b.variant}>{t(b.label)}</Badge></td>
                        <td className="px-4 py-3 text-right">
                          <TableActionButton label={t("Actions")}
                            items={[
                              { label: t("Approve"), onSelect: () => openAction(row, "approved"), disabled: !can("schedule-actions") || row.status === "approved" },
                              { label: t("Complete"), onSelect: () => openAction(row, "completed"), disabled: !can("schedule-actions") || row.status === "completed" },
                              { label: t("Cancel"), onSelect: () => openAction(row, "cancelled"), disabled: !can("schedule-actions") || row.status === "cancelled", destructive: true },
                              { label: t("Delete"), onSelect: () => del(row.id), disabled: !can("delete-schedules"), destructive: true, icon: <Trash2 className="h-4 w-4" /> },
                            ]} />
                        </td>
                      </tr>
                    );
      ...
      <Sheet open={actionOpen} onOpenChange={setActionOpen}>
        <SheetContent className="w-full sm:max-w-[400px] overflow-y-auto flex flex-col gap-0 p-0">
          <SheetHeader className="px-6 py-4 border-b">
            <SheetTitle>
              {actionStatus === "approved" ? t("Approve Schedule") : actionStatus === "completed" ? t("Mark Completed") : t("Cancel Schedule")}
            </SheetTitle>
          </SheetHeader>
          <form onSubmit={applyAction} className="flex flex-col flex-1">
            <div className="flex-1 px-6 py-5 space-y-4">
              {actionRow && (
                <div className="rounded-lg border p-3 bg-muted/30 space-y-1 text-sm">
                  <p><span className="text-muted-foreground">{t("Client:")}</span> {actionRow.name}</p>
                  <p><span className="text-muted-foreground">{t("Date:")}</span> {fmtDate(actionRow.date)}</p>
                  <p><span className="text-muted-foreground">{t("Time:")}</span> {actionRow.startTime ?? "-"}{actionRow.endTime ? ` – ${actionRow.endTime}` : ""}</p>
                </div>
              )}
              {(actionStatus === "cancelled" || actionStatus === "rejected") && (
                <div className="space-y-2">
                  <Label>{t("Reason / Description")}</Label>
                  <Textarea value={cancelDesc} onChange={e => setCancelDesc(e.target.value)} rows={3} placeholder={t("Enter reason...")} />
                </div>
              )}
              {actionStatus === "approved" && (
                <div className="flex items-center gap-2 text-sm text-green-600"><CheckCircle className="h-5 w-5" />{t("This will mark the schedule as approved.")}</div>
              )}
              {actionStatus === "completed" && (
                <div className="flex items-center gap-2 text-sm text-blue-600"><CheckCircle className="h-5 w-5" />{t("This will mark the schedule as completed.")}</div>
              )}
            </div>
            <div className="border-t px-6 py-4 flex justify-end gap-2 bg-background">
              ...
              <Button type="submit" disabled={processing}
                variant={actionStatus === "cancelled" || actionStatus === "rejected" ? "destructive" : "default"}>
