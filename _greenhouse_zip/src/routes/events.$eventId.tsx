...
import { CalendarDays, Clock, MapPin, Ticket, Share2, QrCode, Users, Shirt, Utensils } from "lucide-react";
import {
  events,
  formatEventDate,
  getCompany,
...
export const Route = createFileRoute("/events/$eventId")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [
          { title: "Event not found — Greenhouse Bingo" },
    ...
    return {
      meta: [
        { title: `${event.title} — ${co?.name ?? "Greenhouse Bingo"}` },
